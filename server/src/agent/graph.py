...
async def call_model(state: State ):
    print('call_model b')
    # print(state['messages'])
    response = await llm.bind_tools(tools=tools).ainvoke(state['messages'])

    writer = get_stream_writer()  
    writer({"type": "progress", "step": 1, "status": "Fetching weather..."})
    writer({"type": "progress", "step": 2, "status": "Parsing results"})


    for i in range(1000):
        writer(AIMessage(content='hi'))
...
async def tool_node(state):
    ...
    for tool_call in state['messages'][-1].tool_calls:
            ...
            #ToolMessage list
            tool_message = ToolMessage(content=json.dumps(result),tool_call_id=tool_id,name=tool_name)
